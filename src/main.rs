...
pub mod models;
use reqwest::header::{HeaderMap, HeaderValue, CONTENT_TYPE, AUTHORIZATION, CONTENT_LENGTH, HOST};
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let resp = client.get("https://api.spacetraders.io/v2/my/contracts/clhi30h5t10hfs60dompcbn9p").bearer_auth(token).send().await?;
